...
args.save = 'checkpoints/auto_aug-1500-{}-{}-{}-{}-{}-{}-{}-{}'.format(args.arch, args.init_ch, args.model_ema,
                                                         args.model_ema_decay, args.drop_path_prob,
                                                               args.layers, args.cutout_length, args.save)


print("save path:", args.save)
writer = SummaryWriter(args.save + '/logs')
...
def main():
    ...
    if not continue_train:
        ...
        optimizer = torch.optim.Adagrad(model.parameters(), lr=0.0025, lr_decay=0, weight_decay=3e-4)
        if args.model_ema:
            model_ema = ModelEma(
                model,
        ...
        optimizer = torch.optim.Adagrad(params, lr=0.0025, lr_decay=0, weight_decay=3e-4)
    ...
    for epoch in range(cur_epoch, args.epochs):
        ...
        writer.add_scalar('val_acc/epoch',valid_acc,epoch)
        writer.add_scalar('val_acc_ema/epoch',valid_acc_ema,epoch)
        writer.add_scalar('valid_loss_ema/epoch',valid_obj_ema,epoch)
        writer.add_scalar('valid_loss/epoch',valid_obj,epoch)
...
def train(train_queue, model, criterion, optimizer, epoch, model_ema=None):
    ...
    for step, (x, target) in enumerate(train_queue):
        ...
        # logging.info('target.size: %d', len(target.size()))
        # if len(target.size()) == 1:
        # print(target.size())
        prec1, prec5 = utils.cutmix_accuracy(logits, target, topk=(1, 5))
        n = x.size(0)
    ...
    writer.add_scalar('train_losses.avg/epoch',losses.avg,epoch)
